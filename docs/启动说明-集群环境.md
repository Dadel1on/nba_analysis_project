...
- [application.yml](../backend-spring/src/main/resources/application.yml)

确保 `spring.datasource.url` 指向 `master:3306/nba_stats`（或使用 IP）。当前后端默认配置也已经指向这个地址，必要时可以通过环境变量覆盖。
...
如果要对照仓库源码，脚本原文件位于 [backend-spring/scripts/spark_nba_ml.py](../backend-spring/scripts/spark_nba_ml.py)。
